...
const makeHtml = (
  ...
  { bundleName, html, rnwStyles, scStyles, title }
        ...
        <html>
          ...
          <script="/vendor.bundle.js"></script>
          <script="/${bundleName}.bundle.js"></script>
        </html>
      `;
...
server.get("/profile/:slug", (req, res) => {
  ...
  const client = makeClient(); // only creating apollo client
  const App = authorProfile(client, slug, pageNum);//create the app

  getData(App).then(props => //get the data and other stuff from react component
    res.send(
